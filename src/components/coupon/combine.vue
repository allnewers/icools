...
import { sendLoginCode1,bindMobileToWx,sendWxCode } from "../../api";
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.loginData.memberId = this.id;
      bindMobileToWx(this.loginData)
        .then(res => {
          console.log(res);
          //设置cookie
          // document.cookie = `token=${escape(res.data.token)};`
          if(!CookieEnable()){
            Toast('对不起，您的浏览器cookie功能被禁用，请开启并重启浏览器');
            return;
          }  
          if(res!==null && res.result === false) return;  
         // setCookie('userAvatar',res.data.imgUrl,7);
          Toast('登录成功');
          //location.reload();
          this.$router.push({path:'/coupon',query:{userId:res.id}});
        })
        .catch(err => {
          Toast(err);
        });
